...
def _acc_sd(rows: list[dict], idx: int) -> Optional[float]:
    ...
    for i in range(n):
        ...
        if action is None and position == 0 and cooldown >= COOLDOWN:
            ...
            # 极低位缩量: 低量=卖压耗尽 (无份额过滤, 份额无判别力)
                  ...
                  and chg <= EXTREME_CHG and days_high >= DROP_EARLY_DAYS
                  and accum_count < CLUSTER_ACCUM):
                action = "BUY"
                reason = f"极低位: pp{pp:.0f}+缩量{vr:.2f}"

            # 极低位放量恐慌: 放量恐慌日见底 (2026-07-30 型)
            elif (pp is not None and pp <= PANIC_LOW_PP
                  and chg <= PANIC_LOW_CHG and vr >= PANIC_LOW_VR):
                action = "BUY"
        ...
        if action is None and position == 1:
            ...
            if (is_dist and pp is not None and pp >= DIST_PP_MIN
                    and vr >= DIST_VR_MIN and not memory_exempt
                    and sd_today is not None and sd_today < 0):
                pending_sell_idx = i
            # S2: 破位出货 - 大跌+放量+当日净赎回
            elif (chg <= BREAK_CHG and vr >= BREAK_VR_MIN
