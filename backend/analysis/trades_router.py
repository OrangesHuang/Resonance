...
def _run_default(rows: list[dict], t_pct: dict, m_pct: dict) -> dict:
    ...
    for i, row in enumerate(rows):
        ...
        if action == "BUY":
            ...
            trades.append({"date": d, "action": action, "price": close,
                           "reason": reason})
        elif action == "SELL":
            position = 0.0
            trades.append({"date": d, "action": action, "price": close,
...
def compute_trades(code: str, rows: list[dict], *,
                   t_pct: Optional[dict] = None, m_pct: Optional[dict] = None,
                   hs300_rows: Optional[list[dict]] = None,
                   kc_idx_rows: Optional[list[dict]] = None) -> dict:
    """生成与页面「共振买卖点」一致的交易信号。
