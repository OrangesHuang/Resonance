...
def job_rebuild_all(progress: ProgressFn,
    ...
    etf = job_backfill_etf_daily(_scaled(progress, _W_CALENDAR, _W_ETF), etf_days)

    shares_start = _W_CALENDAR + _W_ETF
    progress(shares_start, 100, "阶段3/4 份额数据")
    shares = job_backfill_shares(_scaled(progress, shares_start, _W_SHARES), shares_days, force)

    sentiment_start = shares_start + _W_SHARES
    progress(sentiment_start, 100, "阶段4/4 市场情绪")
    sentiment = job_fetch_sentiment(_scaled(progress, sentiment_start, _W_SENTIMENT), sentiment_days)
