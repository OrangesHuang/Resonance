...
from base.analysis.sentiment.core import percentile_series, turnover_value
...
def _current_indicators(etf_row: dict, turn_p, margin_p, states: dict) -> list:
    ...
    for ind in INDICATORS:
        ...
        out.append({**ind, "state": states[ind["key"]], "value": value, "display": display, "note": note})
    return out
...
def compute_resonance(
    ...
    min_pts: int = SENTIMENT_ZONE_MIN_PTS,
) -> dict:
    etf_by_date = {r["date"]: r for r in etf_rows}
    t_pct = percentile_series(
        [r.get("date") for r in turnover_rows], [turnover_value(r) for r in turnover_rows], window, min_pts
    )
    m_pct = percentile_series(
        [r.get("date") for r in margin_rows], [r.get("fin_balance_yi") for r in margin_rows], window, min_pts
    )
