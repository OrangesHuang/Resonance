...
def calc_price_position(kline: list[dict], target_idx: int, window: int = POSITION_WINDOW) -> float | None:
...
def calc_price_position_from_price(price: float, kline: list[dict], window: int = POSITION_WINDOW) -> float | None:
    recent = kline[-window:]
    if not recent:
        return None
    hi = max(k["high"] for k in recent)
...
def classify_trade_direction(price_position: float | None, volume_ratio: float | None) -> str:
