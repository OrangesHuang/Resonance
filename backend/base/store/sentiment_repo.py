...
def get_turnover_series(limit: int | None = None) -> list[dict]:
    conn = get_connection()
    try:
...
def get_margin_series(limit: int | None = None) -> list[dict]:
    conn = get_connection()
    try:
        if limit:
            sql = (
            ...
            sql = (
                "SELECT date, fin_balance_yi, loan_balance_yi, fin_buy_yi, source FROM margin_trading ORDER BY date ASC"
            )
            rows = conn.execute(sql).fetchall()
        return [dict(r) for r in rows]
    finally:
...
def get_turnover_latest_date() -> str | None:
    conn = get_connection()
    try:
...
def get_margin_latest_date() -> str | None:
    conn = get_connection()
    try:
