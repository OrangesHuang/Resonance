...
def get_pcr_series(limit: int | None = None) -> list[dict]:
    conn = get_connection()
    try:
...
def get_basis_series(limit: int | None = None) -> list[dict]:
    conn = get_connection()
    try:
...
def get_pcr_latest_date() -> str | None:
    conn = get_connection()
    try:
...
def get_basis_latest_date() -> str | None:
    conn = get_connection()
    try:
