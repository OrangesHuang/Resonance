...
def run_kc50_strategy(kc50_rows, kc_idx_rows):
    """用科创综指的买卖日期映射到科创50价格。"""
    from base.analysis.strategy.kc import run_kc_strategy
    ...
    for t in kc_trades:
        ...
        if d not in kc50_by_date:
            continue
        row = kc50_by_date[d]
        trades.append({"date": d, "action": t["action"], "price": row["close_price"], "reason": "复用科创综指信号"})
    ...
    return {"code": KC50_CODE, "trades": trades, "metrics": metrics, "holding": position > 0}
