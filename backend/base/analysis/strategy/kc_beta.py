...
Beta 新增(每条一个以上案例):
  ...
  2. 加速赶顶卖: 2日累计涨≥6.5% + pp≥99 + 浮盈≥15% → 当日卖。案例:
     2026-06-30 (06-29 +3.5% + 06-30 +4.02%, pp100) → @1.97, 次日
     07-02 -5.3%。06-18 同指纹 pp98 拦(主升中段)。
  3. 洗盘回买: 两日累计跌≥5% + 当日仍跌≥1% + pp≤60 + 距20日高≤-8%
     + 前5日累计净申赎≥-0.5亿 + 前一日 sp≥70 + vr≤1.3 缩量 → 当日买。案例: 2026-06-08
     @1.53 → 06-30 @1.97 +28.8%。06-01 同指纹 pp62.9 拦; 07-13 同指纹
     sp37.6 / 07-16 sp50.2(份额动能塌)、07-17 放量崩盘 vr2.06、07-23 两日
     累计仅-4.5% 拦 = 防接刀。
...
过拟合审计(2026-08-18 敏感性扫描, 16 参数逐档扰动):
  钝感平台(阈值在宽区间内任意取值结果不变): GREEDY_PP_MIN/GREEDY_PROFIT_MIN/
  BLOW_CHG_MIN/BLOW_PROFIT_MIN/WASH_DD20_MIN/WASH_SD5_MIN/WASH_VR_MAX 共 8 个。
  稳健化后: WASH 用两日累计≥5%+当日≥1% 替代两个单日阈值(原 2.5 vs 07-23
  -2.48 只差 0.02, 已消除); BLOW_2D_CHG 6.0→6.5(原 06-18 的 6.08 贴 0.08);
  WASH_SP_MIN 60→70(居中 50.2 与 80.1)。
  复扫确认: BLOW_PP_MIN 97~99 与 BLOW_2D_CHG 5.5~7.5 全区间结果不变
  (06-18 的 pp98/2日6.08 由两条件交叉保护, 单边放松不触发, 不亏钱只少赚);
  WASH_2D_DROP_MIN 平台 4.5~5.8(基线 5.0, 假案例 07-23 -4.48 距 0.5),
  WASH_DAY_CHG_MIN 平台 0.5~3.0, WASH_SP_MIN 平台 55~80(基线 70 居中)。
  结构性保留: S-B/P-W/S1 门槛各仅 1 个正向案例, 需未来新案例验证, 暂不升级正式版。
"""
...
BLOW_2D_CHG = 6.5  # S-B 加速赶顶: 2日累计涨幅(06-30 7.7; 06-18 6.1 主升中段拦)
...
WASH_2D_DROP_MIN = 5.0  # P-W 两日累计跌幅(06-08 -5.8; 07-23 -4.5 假洗盘拦)
WASH_DAY_CHG_MIN = 1.0  # P-W 当日仍在跌(06-08 -3.5; 07-29 -0.3 拦)
...
WASH_SP_MIN = 70.0  # P-W 前一日份额概率(06-08 80; 07-13 37.6 / 07-16 50.2 接刀拦)
...
def run_kc_beta_strategy(rows: list[dict]) -> dict:
    ...
    for i in range(n):
        ...
        if action is None and position == 0 and cooldown >= COOLDOWN:
            ...
            sd5 = _sd5(rows, i)
            wash = (
                chg + prev_chg <= -WASH_2D_DROP_MIN
                and chg <= -WASH_DAY_CHG_MIN
                and pp is not None
                and pp <= WASH_PP_MAX
