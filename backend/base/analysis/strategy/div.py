...
def run_div_strategy(rows: list[dict]) -> dict:
    ...
    for i in range(n):
        ...
        if position == 0 and waiting_for_reversal:
            ...
            two_day_reversal = chg > 0 and prev_chg > 0 and vr > 1.0 and prev_vr > 0.8 and pp_ok
        ...
        if position == 1:
            ...
            if hold_days >= MIN_HOLD and is_dist and dist_count >= sell_threshold:
                reason = f"出货确认({dist_count}/{sell_threshold})+pp{pp:.0f}+vr{vr:.1f}"
                action = "SELL"
    ...
    return {"code": DIV_CODE, "trades": trades, "metrics": metrics, "holding": position > 0}
