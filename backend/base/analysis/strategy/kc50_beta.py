...
核心: 买点 = 价格低位 + 机构净申购(sd>0, 9 组无一例外); 顶两类 —
净赎回顶(连续3日流出+sp<=15+浮盈>=18%)与净申购顶(散户接盘力竭: 放量
触线/加速赶顶/暴跌离场); 单日流出多为波段内洗盘, 需连续+浮盈双确认。
买入(全 sd>0): P1 恐慌 / P3 熊市深回撤(距250日高<=-38%+跌速趋缓) /
P4 牛市回调(pp<=35+sp>=50+距60日高<=-8%) / P5 放量大阳(924式) /
P6 熊市回调(pp15-30+申购>=5亿) / P7 线上极冷(不依赖牛熊判定)。
卖出: S1 止损 -> S-A 净赎回顶 -> S-B 放量触线 -> S-D 滞涨顶 -> S-E 缩量
新高背离 -> S6 暴跌 -> S2 加速赶顶观察 -> S3 累计流出100亿 -> S4 尾随兜底;
尾随与当日买入冲突时买入优先。迭代备忘: 顶背离/浮盈分档/13%尾随过拟合
撤回, 尾随参数不可泛化仅兜底。
"""
...
BEAR_MA60_SLOPE_MIN = -5.0  # P3 跌速门槛(2022-04 -8.5% 接刀必死)
BEAR_PP_MAX = 20.0  # P3 pp 上限
BULL_PP_MAX = 35.0  # P4 牛市回调底 pp 上限(2025-11-21 pp29.6)
BULL_SP_MIN = 50.0  # P4 份额下限(2025-01-06 sp55.7)
BULL_DD60_MIN = 8.0  # P4 距60日高回撤下限(2022-01-19 -4.3% 拦)
...
TOUCH_DIV_MIN = -7.0  # S-B 反弹触线: 距 ma250 7% 内(2022-08-05/11-04)
TOUCH_PROFIT_MIN = 15.0  # S-B 浮盈门槛(2023-01~02 到线浮盈<15% 持有穿越)
