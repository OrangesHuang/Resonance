...
def run_sh50_strategy(rows: list[dict]) -> dict:
    ...
    for i in range(n):
        ...
        if position == 0 and cooldown >= COOLDOWN:
            ...
            # P2: 低位吸筹 (下跌末期 或 国家队大额申购豁免)
                ...
                and accum_count < CLUSTER_ACCUM
                and money_ok
                and (days_high >= DROP_EARLY_DAYS or (sd is not None and sd >= BIG_BUY_YI))
            ):
                action = "BUY"
        ...
        if position == 1:
            ...
            if hold_days >= MIN_HOLD and sd is not None and sd <= REDEEM_YI and pp is not None and pp >= REDEEM_PP_MIN:
                reason = f"巨额赎回: {sd:.1f}亿+pp{pp:.0f}"
                action = "SELL"

            # T1: 持仓期最高收盘回落5%止盈
            elif hold_days >= MIN_HOLD and peak_close > 0 and close < peak_close * (1 - TRAIL_PCT):
    ...
    return {"code": SH50_CODE, "trades": trades, "metrics": metrics, "holding": position > 0}
