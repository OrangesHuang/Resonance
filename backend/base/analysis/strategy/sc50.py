...
def _new_low(rows: list[dict], idx: int, window: int = 20) -> bool:
    ...
    return close < min((rows[j].get("close_price") or 0) for j in range(max(0, idx - window), idx))


def _recent_low_pp(rows: list[dict], idx: int) -> bool:
    """近 BOOST_LOOKBACK 日内(不含当日)出现过极低位(已缩量见底)。"""
    lo = max(0, idx - BOOST_LOOKBACK)
    return any((rows[j].get("price_position") or 99) <= BOOST_PP_LOW for j in range(lo, idx))
...
def run_sc50_strategy(rows: list[dict]) -> dict:
    ...
    for i in range(n):
        ...
        if position == 0 and cooldown >= COOLDOWN:
            ...
            high_30d = max((rows[j].get("close_price") or 0) for j in range(max(0, i - 29), i + 1))
            decline_pct = (high_30d - close) / high_30d * 100 if high_30d > 0 else 0
            if (
                pp is not None
                and pp <= EXTREME_PP_MAX
            ...
            # P1: 单日恐慌 (非集群中, 左侧)
                ...
                is_accum and chg <= PANIC_DROP and pp is not None and pp <= PANIC_PP_MAX and accum_count < CLUSTER_ACCUM
            ):
                action = "BUY"
        ...
        if position == 0 and waiting_reversal and wait_low is not None:
            if close < wait_low:
                ...
                strong_bounce = chg >= BOUNCE_CHG and vr >= BOUNCE_VR and pp_ok
                two_day_up = cluster_ended and chg > 0 and prev_chg > 0 and vr > 1.0 and pp_ok

                if strong_bounce or two_day_up:
                    action = "BUY"
                    reason = (
                        f"右侧反弹: 涨{chg:.1f}%+vr{vr:.1f}+pp{pp:.0f}"
                        if strong_bounce
                        else f"右侧连涨: 2天+放量+pp{pp:.0f}"
                    )
                    waiting_reversal = False
                    wait_low = None
        ...
        if position == 1:
            ...
            if hold_days >= MIN_HOLD and close < peak_close * (1 - TRAIL_PCT):
    ...
    return {"code": SC50_CODE, "trades": trades, "metrics": metrics, "holding": position > 0}
