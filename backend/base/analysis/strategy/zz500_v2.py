...
def _recent_dist(rows: list[dict], idx: int, window: int = 5) -> bool:
    ...
    return any(rows[j].get("trade_direction") == "DISTRIBUTE" for j in range(max(0, idx - window), idx))
...
def run_zz500_strategy_v2(rows: list[dict]) -> dict:
    ...
    for i in range(n):
        ...
        accum_count = sum(
            1 for j in range(max(0, i - CLUSTER_WINDOW + 1), i + 1) if rows[j].get("trade_direction") == "ACCUMULATE"
        )
        ...
        if position == 0 and cooldown >= COOLDOWN:
            ...
            elif pp_panic and chg <= PANIC_DROP and days_high >= PANIC_MATURITY_DAYS and accum_count < CLUSTER_ACCUM:
                action = "BUY"
            ...
            # P3: 低位孤立吸筹 (ACCUMULATE + pp≤35 + 非集群 + 距高点≥15天)
            elif is_accum and pp_low and accum_count < CLUSTER_ACCUM and days_high >= DROP_EARLY_DAYS:
                action = "BUY"
            ...
            # P4: 暴跌集群 → 等待右侧确认
                ...
                wait_low = min((rows[j].get("close_price") or 0) for j in range(max(0, i - CLUSTER_WINDOW + 1), i + 1))
        ...
        if position == 0 and waiting_reversal and wait_low is not None:
            if close < wait_low:
                ...
                strong_bounce = chg >= BOUNCE_CHG and vr >= BOUNCE_VR and pp_ok and money_ok
                two_day_up = cluster_ended and chg > 0 and prev_chg > 0 and vr > 1.0 and pp_ok and money_ok

                if strong_bounce or two_day_up:
                    action = "BUY"
                    reason = (
                        f"右侧反弹: 涨{chg:.1f}%+vr{vr:.1f}+份额{sd:.1f}亿"
                        if strong_bounce
                        else f"右侧连涨: 2天+放量+份额{sd:.1f}亿"
                    )
                    waiting_reversal = False
                    wait_low = None
        ...
        if position == 1:
            ...
            if (
                s1_watch == 0
                and action is None
                and (is_dist and pp is not None and pp >= SELL_PP_EXTREME and not _recent_dist(rows, i))
            ):
            ...
            if s1_watch == 0 and action is None and is_dist and pp_high and vr_ok:
                dist_count += 1
                if hold_days >= MIN_HOLD and dist_count >= sell_threshold and sd is not None and sd < 0:
                    reason = f"出货确认({dist_count}/{sell_threshold})+pp{pp:.0f}+份额{sd:.1f}亿流出"
                    action = "SELL"
    ...
    return {"code": ZZ500_CODE, "trades": trades, "metrics": metrics, "holding": position > 0}
