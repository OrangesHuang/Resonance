    ...
    for i, r in enumerate(rows):
        ...
        if i + 1 >= window:
            ...
            if all(v is not None for v in segment):
                ma = round(sum(v for v in segment if v is not None) / window, 2)
                cur = totals[i]
...
def turnover_summary(rows: list[dict]) -> dict:
    if not rows:
        return {"latest_date": None, "latest_yi": None, "ma5_yi": None, "vol_ratio": None, "volume_state": None}
    last = rows[-1]
    return {
...
def margin_summary(rows: list[dict]) -> dict:
    if not rows:
        return {"latest_date": None, "fin_balance_yi": None, "net_fin_buy_yi": None, "prev_fin_balance_yi": None}
    last = rows[-1]
