...
export default function EtfSignalGrid({ etfs }: { etfs: EtfSignal[] }) {
  ...
  const favEtfs = etfs.filter(e => pinned.includes(e.code))
  const otherEtfs = etfs.filter(e => !pinned.includes(e.code))
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
      {list.map(etf => (
        <SignalCard
          key={etf.code}
          signal={etf}
          favorite={pinned.includes(etf.code)}
          onToggleFavorite={() => togglePin(etf.code)}
