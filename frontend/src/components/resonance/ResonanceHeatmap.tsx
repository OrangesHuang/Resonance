import { useEffect, useMemo, useRef } from 'react'
import ReactECharts from 'echarts-for-react'
import type { ECharts } from 'echarts'
import { Rect as ZRect } from 'zrender'
import type { ResonanceOverview, LightState } from '../../api/types'
import { useChartSync, RESONANCE_SYNC_GROUP } from '../../hooks/useChartSync'
import type { AxisPointerBridge } from '../../hooks/useAxisPointerBridge'

const AXIS_LABEL = '#6b7280'

const STATE_VALUE: Record<LightState, number> = { red: 2, green: 1, gray: 0 }
const VALUE_LABEL: Record<number, string> = { 2: '红灯', 1: '绿灯', 0: '中性' }

interface HeatmapTooltipParam {
  value: [number, number, number]
}

interface RenderApi {
  value: (idx: number) => number
  coord: (v: [number, number]) => [number, number]
  getWidth: () => number
  getHeight: () => number
}

export default function ResonanceHeatmap({ data, selectedDate, onSelect, bridge }: {
  data: ResonanceOverview
  selectedDate?: string | null
  onSelect?: (date: string, indicatorKey: string) => void
  bridge?: AxisPointerBridge
}) {
  const connectReady = useChartSync(RESONANCE_SYNC_GROUP)
  const instRef = useRef<ECharts | null>(null)
  const hoverLineRef = useRef<ZRect | null>(null)
  const history = data.history
  const keys = data.indicators.map(i => i.key)
  const names = data.indicators.map(i => i.name)
  const dates = useMemo(() => history.map(h => h.date), [history])
  const datesRef = useRef(dates)
  datesRef.current = dates

  // 订阅 bridge 广播: heatmap 无法渲染 axisPointer 白线, 用 zrender Rect
  // 直接画竖线(不经 setState/setOption, 避免高频 mousemove 触发
  // "setOption should not be called during main process" 报错)
  useEffect(() => {
    if (!bridge) return
    return bridge.subscribe((d: string | null) => {
      try {
        const inst = instRef.current
        if (!inst) return
        const line = hoverLineRef.current
        if (!d) {
          if (line) line.attr({ style: { opacity: 0 } })
          return
        }
        const idx = datesRef.current.indexOf(d)
        if (idx < 0) {
          if (line) line.attr({ style: { opacity: 0 } })
          return
        }
        const px = inst.convertToPixel({ xAxisIndex: 0 }, idx) as number | null
        if (px == null || Number.isNaN(px)) return
        // 可视范围检查: 缩放后窗口外的索引 convertToPixel 会返回
        // 越界像素(负数/超宽), 白线会画到最左侧, 需隐藏
        const w = inst.getWidth()
        if (px < 0 || px > w) {
          if (line) line.attr({ style: { opacity: 0 } })
          return
        }
        if (!line) {
          const zr = inst.getZr()
          if (!zr) return
          hoverLineRef.current = new ZRect({
            shape: { x: px - 1, y: 0, width: 2, height: inst.getHeight() },
            style: { fill: '#9ca3af' },
            silent: true,
            z: 100,
          })
          zr.add(hoverLineRef.current as never)
        } else {
          line.attr({ shape: { x: px - 1 }, style: { opacity: 1 } })
        }
      } catch {
        // 忽略
      }
    })
  }, [bridge])

  const onChartReady = (inst: ECharts) => {
    instRef.current = inst
    // 实例重建(dispose 后新实例)时旧 Rect 已随旧 zr 销毁: 重置引用,
    // 否则后续 attr 操作死元素且永不重建(白线消失)
    hoverLineRef.current = null
    connectReady(inst)
    bridge?.register(inst, () => datesRef.current)
  }

  // option 用 useMemo 缓存: 缩放/dateWindow 变化不重建(避免 setOption 冲突)
  const option = useMemo(() => {
    // notMerge 重建会重置 dataZoom: 从实例实时读当前缩放回填
    const dz = instRef.current?.getOption().dataZoom as
      | Array<{ start?: number; end?: number }>
      | undefined
    const zStart = dz?.[0]?.start ?? 0
    const zEnd = dz?.[0]?.end ?? 100
    type HeatCell = [number, number, number] | {
      value: [number, number, number]
      itemStyle: { borderColor: string; borderWidth: number; shadowBlur: number; shadowColor: string }
    }
    const heatData: HeatCell[] = []
    const selectedCells: HeatCell[] = []
    history.forEach((h, x) => {
      const isSel = selectedDate != null && h.date === selectedDate
      keys.forEach((key, y) => {
        const st: LightState = h.states[key] ?? 'gray'
        const v: [number, number, number] = [x, y, STATE_VALUE[st]]
        if (isSel) {
          selectedCells.push({ value: v, itemStyle: { borderColor: '#38bdf8', borderWidth: 2, shadowBlur: 10, shadowColor: 'rgba(56, 189, 248, 0.9)' } })
        } else {
          heatData.push(v)
        }
      })
    })
    heatData.push(...selectedCells)

    return {
      backgroundColor: 'transparent',
      animation: false,
      tooltip: {
        transitionDuration: 0,
        trigger: 'axis',
        backgroundColor: '#111827',
        borderColor: '#374151',
        textStyle: { color: '#e5e7eb' },
        axisPointer: {
          type: 'line',
          lineStyle: { color: '#9ca3af', type: 'dashed', width: 1 },
        },
        formatter: (param: HeatmapTooltipParam | HeatmapTooltipParam[]) => {
          // trigger:'axis' 时 formatter 收到数组(axis 上所有数据点)
          const p = Array.isArray(param) ? param[0] : param
          if (!p || !p.value) return ''
          const [x, y, v] = p.value
          return `${dates[x]}<br/>${names[y]}：${VALUE_LABEL[v] ?? '中性'}`
        },
      },
      axisPointer: { link: [{ xAxisIndex: 'all' }] },
      grid: { left: 80, right: 20, top: 40, bottom: 60 },
      xAxis: {
        type: 'category',
        data: dates,
        boundaryGap: true,
        splitArea: { show: false },
        axisLabel: { color: AXIS_LABEL, fontSize: 10 },
      },
      yAxis: {
        type: 'category',
        data: names,
        inverse: true,
        splitArea: { show: false },
        axisLabel: { color: AXIS_LABEL, fontSize: 11 },
      },
      visualMap: {
        type: 'piecewise',
        dimension: 2,
        pieces: [
          { value: 2, label: '红灯', color: '#ef4444' },
          { value: 1, label: '绿灯', color: '#22c55e' },
          { value: 0, label: '中性', color: '#374151' },
        ],
        orient: 'horizontal',
        left: 80,
        top: 5,
        itemWidth: 12,
        itemHeight: 12,
        textStyle: { color: AXIS_LABEL, fontSize: 10 },
      },
      series: [
        {
          name: '共振状态',
          type: 'heatmap',
          data: heatData,
          itemStyle: { borderColor: '#0a0f1a', borderWidth: 1 },
          emphasis: { itemStyle: { borderColor: '#e5e7eb', borderWidth: 1 } },
        },
        // 选中高亮列: custom series 放大整列(蓝框)
        ...(selectedDate != null && dates.includes(selectedDate) ? [{
          name: '选中高亮',
          type: 'custom' as const,
          xAxisIndex: 0,
          yAxisIndex: 0,
          data: keys.map((_, y) => [dates.indexOf(selectedDate), y]),
          silent: true,
          z: 10,
          renderItem: (_params: unknown, api: RenderApi) => {
            const xIdx = api.value(0)
            const yIdx = api.value(1)
            const [cx, cy] = api.coord([xIdx, yIdx])
            // 实时读实例 dataZoom(connect 组同步缩放后 dateWindow 不更新)
            const dz = instRef.current?.getOption().dataZoom as
              | Array<{ start?: number; end?: number }>
              | undefined
            const start = dz?.[0]?.start ?? 0
            const end = dz?.[0]?.end ?? 100
            const visibleRatio = Math.max((end - start) / 100, 0.001)
            const colWidth = api.getWidth() / (dates.length * visibleRatio)
            const cellW = Math.max(colWidth, 10)
            const rowH = keys.length > 1
              ? Math.abs(api.coord([0, 1])[1] - api.coord([0, 0])[1])
              : api.getHeight()
            const st: LightState = history[xIdx]?.states[keys[yIdx]] ?? 'gray'
            const color = st === 'red' ? '#ef4444' : st === 'green' ? '#22c55e' : '#374151'
            return {
              type: 'rect',
              shape: { x: cx - cellW / 2, y: cy - rowH / 2, width: cellW, height: rowH - 1 },
              style: { fill: color, stroke: '#38bdf8', lineWidth: 1.5 },
            }
          },
        }] : []),
      ],
      dataZoom: [
        { type: 'inside', xAxisIndex: 0, start: zStart, end: zEnd },
        {
          type: 'slider',
          xAxisIndex: 0,
          start: zStart,
          end: zEnd,
          bottom: 8,
          height: 18,
          borderColor: '#374151',
          backgroundColor: '#111827',
          fillerColor: 'rgba(75, 85, 99, 0.3)',
          handleStyle: { color: '#6b7280' },
          textStyle: { color: '#6b7280' },
        },
      ],
    }
  }, [history, dates, keys, names, selectedDate])

  // 缩放同步: 靠 connect 组(同 dates 图)+ K线 bridge.zoom 广播;
  // 不再自行 dispatch dateWindow(避免窗口横跳循环)

  const onEvents = {
    click: (params: HeatmapTooltipParam | HeatmapTooltipParam[]) => {
      // trigger:'axis' 且多 series 时 click params 是数组(首项为 heatmap 数据点)
      const p = Array.isArray(params) ? params[0] : params
      if (!p || !p.value) return
      const [x, y] = p.value
      const date = dates[x]
      const key = keys[y]
      if (date && key) onSelect?.(date, key)
    },
    updateAxisPointer: (e: { x?: number }) => {
      if (!bridge || e.x == null) return
      try {
        const px = instRef.current?.convertFromPixel({ xAxisIndex: 0 }, e.x) as number | null
        const idx = px != null && !Number.isNaN(px) ? Math.round(px) : -1
        if (idx >= 0 && idx < dates.length) bridge.show(dates[idx], instRef.current)
      } catch {
        // 忽略
      }
    },
    // 缩放由 echarts.connect 原生组分发, 不再上报父级
  }

  if (history.length === 0 || keys.length === 0) {
    return <div className="text-gray-500 text-center py-10">暂无数据</div>
  }

  return (
    <ReactECharts
      onChartReady={onChartReady}
      option={option}
      onEvents={onEvents}
      style={{ height: 260 }}
      notMerge
    />
  )
}